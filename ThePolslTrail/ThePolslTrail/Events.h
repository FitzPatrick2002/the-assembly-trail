...
void medbayGlass() {
	...
	if (c == 'y' || c == 'Y')
	{
		...
		else
		{
			...
			changeHP(utils::valueFromRange(-3, 0));
		}
...
void medbayAsh() {
	...
	if (c == 'y' || c == 'Y')
	{
		...
		else if (c == '2')
		{
			...
			changeHP(utils::valueFromRange(-5, -1));
		}
		else if (c == '3')
		{
	...
	else
	{
		...
		changeHP(utils::valueFromRange(-3, 3));
	}
}
...
void alienHide() {
	...
	if ((c == '1' || c == '2') && utils::fitTimeframe(start, end)) //managed to write correct input in a specified timeframe
	{
		...
		if (player.roomHidingLocations[0] == hidingLocations[c - 49]) //typed in correctly 49 - ascii and then -1 beceause of index
		{
			...
			changeHP(utils::valueFromRange(-6, 0));
		}
	}
...
void WCHiddenPassage() {
	...
	if (c == 'y' || c == 'Y')
	{
		...
		else if (c == 'n' || c == 'N')
		{
			...
			changeHP(utils::valueFromRange(-6, 0)); changeWater(-1);
		}
	}
	else if (c == 'n' || c == 'N')
	{
